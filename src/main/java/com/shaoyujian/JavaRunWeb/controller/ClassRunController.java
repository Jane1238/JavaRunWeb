...
public class ClassRunController {
	...
	@RequestMapping("/runClass")
	@ResponseBody
	public StringBuffer runClass(HttpServletRequest req) throws IOException, ServletException {
...
//				req.setAttribute("msg", "编译失败：格式不符合规范，请检查类名是否正确(如：public class YouClassName{})");
				...
				bw.write("javap -verbose " + className + " \n");
				bw.close();
				...
				//建立个线程池将主进程中的执行结果放入到加到StringBuffer字符串中，与下面的运行时间一并传给前端
...
//							req.setAttribute("arg", "运行成功,运行时间为:" + useTime+"毫秒");
			...
			e.printStackTrace();
			res.append( "编码格式有误！无法生成字节码！请【点击运行】查看错误原因！");
		}
		return res;
	}
	...
	public class Cc implements Callable<String>{
		...
		public String call() throws Exception {
			String res ="";
			BufferedReader br = new BufferedReader(new InputStreamReader(process.getInputStream(),"GBK"));//文件输入流，读取主进程编译完的字节码结构
			//BufferedReader be = new BufferedReader(new InputStreamReader(process.getErrorStream(),"GBK"));
...
//				be.close();
				if (i == 0) {
					while ((content = br.readLine()) != null) {
						...
						pr = sb.toString();
						pr = pr.substring(pr.indexOf(className+".class") + (6 + className.length()), pr.lastIndexOf("SourceFile"));
